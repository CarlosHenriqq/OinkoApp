...
async function carregarUsuario() {
    try {
        ...
        if (userId) {
            console.log('Fazendo requisição para:', `${API_BASE_URL}${ENDPOINTS.USER_INFO}`);
            ...
            const { nome, email, renda, foto_perfil_url } = response.data;
            ...
            if (foto_perfil_url) {
                const fotoCompleta = foto_perfil_url.startsWith('/uploads')
                    ? `${API_BASE_URL}${foto_perfil_url}`
                    : foto_perfil_url;

                setFotoUri(fotoCompleta);
            }
        }
    } catch (error) {
        console.error('Erro ao carregar usuário no perfil:', JSON.stringify(error, null, 2));
    }
}
